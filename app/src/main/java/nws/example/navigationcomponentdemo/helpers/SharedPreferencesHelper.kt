package nws.example.navigationcomponentdemo.helpers
...
class SharedPreferencesHelper private constructor() {
    ...
    operator fun <T> get(key: String, anonymousClass: Class<T>): T {
        return when (anonymousClass) {
            ...
            else -> MainApplication.self().getGson().fromJson(
    ...
    fun <T> put(key: String, data: T) {
        ...
        when (data) {
            ...
            else -> editor.putString(key, MainApplication.self().getGson().toJson(data))
        }
        editor.apply()
    }
    ...
    companion object {
        ...
        private const val PREFS_NAME = "shared_preferences_helper"
